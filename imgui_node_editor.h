...
IMGUI_NODE_EDITOR_API bool
    Link(LinkId id, PinId startPinId, PinId endPinId, const ImVec4& color = ImVec4(1, 1, 1, 1), float thickness = 1.0f);
...
IMGUI_NODE_EDITOR_API void
    SetNodeZPosition(NodeId nodeId, float z); // Sets node z position, nodes with higher value are drawn over nodes with lower value
...
IMGUI_NODE_EDITOR_API bool
    GetLinkPins(LinkId linkId, PinId* startPinId, PinId* endPinId); // pass nullptr if particular pin do not interest you

IMGUI_NODE_EDITOR_API bool PinHadAnyLinks(PinId pinId);

IMGUI_NODE_EDITOR_API ImVec2 GetScreenSize();
// Min.x, Min.y, Max.x, Max.y
IMGUI_NODE_EDITOR_API ImVec4 GetViewScreenPos();
